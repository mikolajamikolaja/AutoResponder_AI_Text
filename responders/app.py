...
# ── Globalny licznik aktywnych pipeline'ów i lista wątków ────────────────────
import threading as _threading
_pipeline_lock    = _threading.Lock()
_active_pipelines = 0
_active_threads: list = []   # śledzimy wątki żeby móc na nie poczekać przy shutdown


def _pipeline_start(thread: "_threading.Thread | None" = None):
...
def _pipeline_done(thread: "_threading.Thread | None" = None):
    global _active_pipelines
    with _pipeline_lock:
        _active_pipelines = max(0, _active_pipelines - 1)
        if thread is not None and thread in _active_threads:
            _active_threads.remove(thread)


def _wait_for_pipelines(timeout: float = 60.0):
    """
    Czeka na zakończenie wszystkich aktywnych pipeline'ów.
    Wywoływane przy atexit — zapobiega zabiciu wątków przez Render.
    """
...
# ── Graceful shutdown — czekaj na pipeline'y przy wyłączaniu procesu ─────────
atexit.register(_wait_for_pipelines, 55.0)


def _sigterm_handler(signum, frame):
    """
    Render wysyła SIGTERM przed hibernacją/restartem.
    Czekamy do 55s na dokończenie pipeline'u zamiast ginąć natychmiast.
    """
    app.logger.warning("[shutdown] SIGTERM — czekam na aktywne pipeline'y (max 55s)...")
    _wait_for_pipelines(55.0)
    app.logger.warning("[shutdown] SIGTERM — pipeline'y zakończone, wychodzę.")
    raise SystemExit(0)
...
try:
    ...
    pass  # nie zawsze możliwe (np. w wątkach potomnych)
...
def pipeline_status_admin():
    """
    Diagnostyczny endpoint — pokazuje aktywne pipeline'y i wątki.
    Użyj gdy log mówi ZŁY (0%) — sprawdź czy wątek w ogóle żyje.
    """
    with _pipeline_lock:
        threads_info = [
            {
    ...
    return jsonify({
        ...
        "tracked_threads":  len(threads_info),
...
def webhook():
    ...
    def _pipeline_wrapper():
        try:
            ...
            app.logger.error(
                "[pipeline] ❌ NIEOCZEKIWANY WYJĄTEK w pipeline — "
                "message_id=%s sender=%s error=%r",
                message_id, sender, _exc,
                exc_info=True,
            )
        finally:
            _pipeline_done(t)
    ...
    t = threading.Thread(target=_pipeline_wrapper, daemon=False)
    _pipeline_start(t)
