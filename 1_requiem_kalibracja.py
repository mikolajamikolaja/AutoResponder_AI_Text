...
def _deepseek_local(system: str, user: str, model=None, timeout=60):
    """Lokalne wywolanie DeepSeek przez OpenAI SDK."""
...
def _build_smierc_raport(state: dict, log: list) -> str:
    ...
    for key, label in [
        ("pawel_1_6", "ETAP 1-6 — Pawel z zaswiatow"),
        ("pawel_7",   "ETAP 7   — Reinkarnacja"),
        ("wyslannik", "ETAP 8+  — Wyslannik"),
    ]:
        if state.get(key):
            lines += [
                f"--- {label} ---",
...
def _test_smierc(body: str, resp_dir: Path) -> dict:
    ...
    result["status"] = f"etap={result.get('nowy_etap','?')} image={img_ok}"
    return result
...
class RequiemApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("✦ REQUIEM KALIBRACJA v6 ✦")
    ...
    def _build_ui(self):
        ...
        # ── Naglowek ─────────────────────────────────────────────────────────
        tk.Label(f, text="✦  REQUIEM  KALIBRACJA  v6  ✦",
        ...
        tk.Label(cfg, text="ETAP PAWLA (1-6)", font=FONT_FILE,
        ...
        # ── 2 Pawel 1-6 ───────────────────────────────────────────────────────
        self._title(f, "2  ETAP 1-6 — Pawel z zaswiatow")
        ...
        # ── 3 Pawel 7 ─────────────────────────────────────────────────────────
        self._title(f, "3  ETAP 7 — Reinkarnacja / pozegnanie")
        ...
        # ── 4 Wyslannik ───────────────────────────────────────────────────────
        self._title(f, "4  ETAP 8+ — Wyslannik z wyzszych sfer")
