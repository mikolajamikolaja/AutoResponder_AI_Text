...
def _generate_photos_parallel(
    ...
    if not get_active_tokens():
        ...
        if sub:
            ...
            # Dwa osobne dict-y — ten sam base64 ale różne nazwy plików,
            # żeby BytesIO przy insert_photo czytało od nowa za każdym razem
            sub_dict_1 = {
                "base64": sub.get("base64"),
                "content_type": "image/jpeg",
                "filename": f"zastepczy_pacjent_{ts_now}.jpg",
            }
            sub_dict_2 = {
                "base64": sub.get("base64"),
                "content_type": "image/jpeg",
                "filename": f"zastepczy_przedmioty_{ts_now}.jpg",
            }
            return sub_dict_1, sub_dict_2
        return None, None
...
def build_raport(
    ...
    # Guard: dane_pacjenta musi być dict — AI może zwrócić listę zamiast dict
    _dp = raport.get("dane_pacjenta", {})
    if not isinstance(_dp, dict):
        current_app.logger.warning(
            "[psych-raport] dane_pacjenta zły typ przed budowaniem nazwy: %s — używam {}",
            type(_dp).__name__,
        )
        _dp = {}
    imie = _dp.get("imie_nazwisko", "pacjent")
