...
Struktura wierszy w zakładce "Historia":
  ...
  Col C: timestamp (ISO)
  Col D: temat
  Col E: status_gas    (ODEBRANO / WEJŚCIE / ODPOWIEDŹ)
...
def get_unprocessed_message_ids(sheet_id: str, max_age_hours: int = 24) -> list:
    """
    Zwraca listę message_id które mają wiersz ODEBRANO ale NIE mają WYSŁANO.
    ...
    try:
        ...
        for row in rows[1:]:  # pomiń nagłówek
            ...
            status_rend = (row[5] or "").strip().upper()
            ...
            if status_rend == "WYSŁANO":
                wyslano_ids.add(msg_id)

        # Zwróć tylko te które mają ODEBRANO ale nie mają WYSŁANO
        unprocessed = []
        for msg_id, info in odebrano.items():
            if msg_id not in wyslano_ids:
                unprocessed.append({
                    "message_id": msg_id,
                    "sender":     info["sender"],
        ...
        logger.info(
            "[sheets_logger] Nieobsłużone wiadomości: %d z %d ODEBRANO",
