...
def log_odebrano(
    sheet_id: str, message_id: str, sender: str, subject: str, body: str
) -> bool:
    """
...
def log_wyslano(
    sheet_id: str, message_id: str, responder: str, tresc_html: str
) -> bool:
    """
...
def get_unprocessed_message_ids(sheet_id: str, max_age_hours: int = 24) -> list:
    ...
    try:
        ...
        for row in rows[1:]:  # pomiń nagłówek
            ...
            if (
                status_gas == "PRZYJETO"
                or status_rend == "WYSŁANO"
                or status_rend.upper().find("GAS_FALLBACK") != -1
                or status_rend.upper().startswith("ERROR:")
                or status_rend.upper().startswith("EMPTY:")
            ):
